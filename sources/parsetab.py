...
_lr_signature = 'ALIAS AND BREAK CASE CLASS DEF DEFINED DIVIDE DO ELSE ELSIF END ENSURE EQUAL FALSE FOR GREATERTHAN GREATERTHANEQUAL IF IN LESSERTHAN LESSERTHANEQUAL LPAREN MINUS MOD MODULE NEXT NIL NOT NOTEQUAL NUMBER OR PLUS PUTS REDO RESCUE RETRY RETURN RPAREN SELF SUPER SYMBOL THEN TIMES TRUE UNDEF UNLESS UNTIL VAR_CLASS VAR_GLOBAL VAR_INSTANCE VAR_LOCAL VAR_SYSTEM WHEN WHILE YIELD _FILE_ _LINE_ beginexpression : expression PLUS termexpression : expression MINUS termexpression : termterm : term TIMES factorterm : term DIVIDE factorterm : factorfactor : NUMBERfactor : LPAREN expression RPAREN'
...
_lr_productions = [
  ...
  ('expression -> expression MINUS term','expression',3,'p_expression_minus','sintactico.py',12),
  ('expression -> term','expression',1,'p_expression_term','sintactico.py',16),
  ('term -> term TIMES factor','term',3,'p_term_times','sintactico.py',20),
  ('term -> term DIVIDE factor','term',3,'p_term_div','sintactico.py',24),
